...
func (c *CachedMetadataResolver) ResolveMembers(ctx context.Context, dim string, codes []string, branchId string) ([]planner.MemberInfo, error) {
	key := c.keyFor("members", dim, codes, branchId)
	if v, ok := c.get(key); ok {
		return v.([]planner.MemberInfo), nil
	}
	res, err := c.inner.ResolveMembers(ctx, dim, codes, branchId)
	if err != nil {
		return nil, err
	}
	c.set(key, res)
...
func (c *CachedMetadataResolver) keyFor(prefix, dim string, codes []string, branchId string) string {
	...
	sb.WriteString(dim)
	sb.WriteString("|")
	sb.WriteString(branchId) // Git-Flow isolation
	sb.WriteString("|")
